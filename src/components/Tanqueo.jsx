...
export function Tanqueo() {
    ...
    useEffect(() => {
            fetch(`http://localhost:8080/vehiculos/mis_vehiculos/${user._id}`, {
            method: "GET"
        }).then(res => res.json())
            .then(res => {
                if (res.status === 'ok') {
    ...
    const cant_gasRef = useRef();
    const tipo_gasRef = useRef();
    const estacion = "Calle 13";
    ...
    async function generarFactura() {
        ...
        const cantidad_gas = cant_gasRef.current.value;
        const tipo_gas = tipo_gasRef.current.value;
        const pago = cantidad_gas * 8000;

        await fetch("http://localhost:8080/tanqueo/factura", {
            headers: {
                "Content-Type": "application/json"
            },
            method: "POST",
            body: JSON.stringify({ placa, cantidad_gas, tipo_gas, estacion, pago, usuario })
        }).then(res => res.json())
        .then(res => {
            if (res.status === "ok") toast.success(res.msg);
                ...
                <Row style={{marginLeft:"40px"}}>
                    <Col>
                        ...
                        <Row>
                            ...
                            <Form.Select ref={placaRef}>
                                {listado.map (v => <option key={v._id} value={v.placa}>{v.placa}</option>)}
                            </Form.Select>
                            <label>Cantidad Gasolina</label>
                            <input ref={cant_gasRef} type="number" />
                            <label>Tipo Gasolina</label>
                            <Form.Select ref={tipo_gasRef} placeholder=" ">
                                <option value="corriente">corriente</option>
                                <option value="corriente">extra</option>
                            </Form.Select>
                            <Button onClick={generarFactura} id="Boton3" as="input" type="submit" value="Facturar" />
                            {
                                listado.map(p => 
                                    <Container key={p._id} value={p._id} id="Cont-Tanqueo">
                                <Row>
